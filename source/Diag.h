...
/**
    ...
    A pattern that never fills fails in ways an operator cannot tell apart
    from the front. The cursor moves and nothing is written. That one symptom
    covers several different faults, and nothing but a log distinguishes them:

    - **A shader would not compile.** The vendor, renderer and version strings
      go next to it, because a shader that builds on one machine and not
    ...
    - **No audio reached the plugin.** Resolume fills an `FF_USAGE_FFT` buffer
      ...
      correct tracker writes nothing -- which looks exactly like a broken one.
      The log records the presence of audio as a *transition*, so a session
      that never saw a signal says so in one line.
    - **The host clock unit.** Resolume sends milliseconds and an offline
      ...
      the clock settled on is stated outright. A clock a thousand times fast
      runs the cursor through the pattern in a tenth of a second.
    - **Which tempo the clock is following**, and what it is: the host's, a
      detected one, or the control. Logged as a transition too.
    ...
    so a pattern that sits silent for an hour writes one line, not 180,000.
*/
namespace pattern::diag
{
